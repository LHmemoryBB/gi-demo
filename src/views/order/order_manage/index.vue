<script setup>
	...
	import { setOrderInfoList, setOrderInfoToBlack, setOrderInfoChangeStatus, setOrderCallBack, getOrderInfoExport,setOrderBuyBack } from '@/api/index'
	
	//解构出复制方法
	const { toClipboard } = clipboard3();
	const ComOrderLog = defineAsyncComponent(() => import('./components/orderLog.vue'))
	const ComVoucher = defineAsyncComponent(() => import('@/views/interface/interface_manage/components/interfaceProductPreview.vue'))
	...
	const onBatch = (type) => {
		...
		openChangeStatus(setOrderInfoChangeStatus,{ids:ids,orderStatus:type}, type == 2 ?'确定要批量成功吗？':'确定要批量失败吗？').then(()=>{
			submit('form')
		})
	}
