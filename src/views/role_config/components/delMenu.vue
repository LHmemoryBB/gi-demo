...
import { roleDelete } from '@/api/index'
...
//关闭
const { loading, data, onSuccess, onError, send } = useAxios(roleDelete, {
  immediate: false
})
onSuccess((res) => {
...
const confirm = () => {
  if (menuName.value !== ruleForm.name) {
    return message.warning('请正确输入要删除的角色标识')
  }
  submit()
}
...
<template>
  <Tdialog :Update="Update" :loading="loading" @confirm="confirm">
    <a-alert message="为防止误操作，请在输入框中输入要删除的角色标识，再进行删除" type="warning" />
    <a-input v-model:value="ruleForm.name" disabled style="width:200px;margin:10px"></a-input>
