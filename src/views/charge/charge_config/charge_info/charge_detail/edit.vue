...
const tableDataDel = () => {
  ...
  ruleForm.chargeConfigs = ruleForm.chargeConfigs.filter(
    (row) => !list.includes(row)
  );
};
...
let showTable1 = ref(false);
let showTable2 = ref(false);
const getselectOptions = () => {
  const param = {
    userId: ruleForm.userId,
    userKey: ruleForm.userKey,
  };
  getSelectOperatorSend(param);
  getSelectPlatFormSend(param);
};
const {
  onSuccess: getSelectPlatFormS,
  onError: getSelectPlatFormE,
  send: getSelectPlatFormSend,
} = useAxios(getSelectPlatForm, {
  immediate: false,
});
getSelectPlatFormS((res) => {
  platFormOptions.value = res.data;
  showTable1.value = true;
});
getSelectPlatFormE((res) => {
  ElNotification.error({
    title: "提示",
...
const {
  onSuccess: getSelectOperatorS,
  onError: getSelectOperatorE,
  send: getSelectOperatorSend,
} = useAxios(getSelectOperator, {
  immediate: false,
});
getSelectOperatorS((res) => {
  operatorOptions.value = res.data;
  showTable2.value = true
});
getSelectOperatorE((res) => {
  ElNotification.error({
    title: "提示",
    message: res.message || "数据获取失败！",
    duration: 3000,
...
const on_init = (ruleform, type) => {
  ruleForm = Object.assign(ruleForm, ruleform);
  if ("" != ruleform.agreement) {
    ruleForm.agreement = [
      {
        name: ruleform.agreement ?? "",
        uid: ruleform.agreement ?? "",
        url: ruleform.agreement ?? "",
        response: [ruleform.agreement ?? ""],
      },
    ];
  }
  if (ruleForm.userId && ruleForm.userKey) {
    getselectOptions();
  }
  setTimeout(()=>{
    Update.isShow = true;
        ...
        >
          ...
          <template #selectPar="{ scope }">
            <Tselect
              ...
              :listInput="{
                ...
                _label: 'value',
                placeholder: '请选择面值',
                default_not_request: !scope.row.platForm && !scope.row.operator,
          ...
          <template #inviteProfit="{ scope }">
            <el-input-number
              ...
              :min="0"
              :max="getMaxInviteProfit(scope.row)"
              :precision="2"
