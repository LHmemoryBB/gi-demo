...
import { orderChargeInfoList, orderChargeInfoExport, chargeBuyback, orderChargeInfoDetail } from '@/api/index'
...
const { loading, data, send, pages, onSuccess } = useAxios(orderChargeInfoList, {
...
const open = () => {
  ...
  openExport(orderChargeInfoExport, data)
}
const {
...
} = useAxios(orderChargeInfoDetail, {
  immediate: false
})
...
<template>
  <div class="page-container">
    ...
    <div class="table-container">
      <a-button type="primary" ghost @click="open()"><DownloadOutlined />导出数据</a-button>
      ...
      <Ttable ref="TtableRef" :loading="loading" :headers="headers" :tableData="tableData" :pages="pages">
        <template #orderStatusStr="{ record }">
          <a-tag :type="getStr(record.orderStatus)">{{ record.orderStatusStr }}</a-tag>
        </template>
        ...
        <template #operater="{ record }">
          <a-button type="primary" ghost v-if="record.orderStatus === 2" @click="buyBackOrder(record.id)" size="samll"
            >返销</a-button
          >
          <a-button
            type="primary"
            ghost
            @click="changeOrder(record)"
            :loading="OrderDetailLoading"
            size="samll"
            class="m_l_10"
            >查看</a-button
