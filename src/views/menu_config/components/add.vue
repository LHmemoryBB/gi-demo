...
import { roleMenuInsert } from '@/api/index'
...
const { loading, data, onSuccess, onError, send } = useAxios(roleMenuInsert, {
  immediate: false
})
onSuccess((res) => {
  ...
  <Tdialog :Update="Update" :loading="loading" @confirm="confirm" @close="close">
    ...
    <template #menuSort="scope" >
      <a-form-item label="排序" name="menuSort">
        <a-input-number v-model:value="ruleForm.menuSort" :rules="{ required: true, message: '请输入排序' }" :min="1" style="width:200px" placeholder="请输入排序"/>
      </a-form-item>
    </template>
    </Tform>
  </Tdialog>
