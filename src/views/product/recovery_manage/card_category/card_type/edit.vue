<script setup>
	...
	import { up_image,setCardTypeEdit } from '@/api/index'
	...
	const { loading, data, onSuccess, onError, send } = useAxios(setCardTypeEdit,{
		immediate:false
	});
	onSuccess((res)=>{
		...
		<Tform ref="RefTform" :loading="loading" :ruleForm="ruleForm" :listInput="listInput" @submit="submit" label_width="120px" btn_hide>
			<template #discount="scope">
				<a-form-item label="填写折扣" name="discount" label-width="120px">
					<a-switch
					    v-model:checked="ruleForm.discount"
						checked-children="是"
			...
			<template #valid="scope">
				<a-form-item label="填写有效期" name="valid" label-width="120px">
					<a-switch
					    v-model:checked="ruleForm.valid"
						checked-children="是"
						un-checked-children="否"
					  />
				</a-form-item>
			...
			<template #validMinDay="scope">
				<a-form-item v-if="ruleForm.valid==true" label="最小有效期" prop="validMinDay" :rules="{ required: true, message: '请填写最小有效期' }" label-width="120px">
					<a-input style="width: 200px;" placeholder="请填写最小有效期" v-model:value="ruleForm.validMinDay" ></a-input>
				</a-form-item>
			</template>
			<template #background="scope">
				<a-form-item label="背景图标" name="background" :rules="{ required: true, message: '请添加图片' }" label-width="120px">
